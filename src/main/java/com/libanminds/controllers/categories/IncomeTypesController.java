...
import com.libanminds.controllers.dialogs.IncomeTypesDialogController;
...
public class IncomeTypesController implements Initializable {
    ...
    private void handleAuthorization() {
        if (Authorization.cannot(AuthorizationKeys.ADD_INCOMES_CATEGORIES))
            buttonsHolder.getChildren().remove(addType);
        if (Authorization.cannot(AuthorizationKeys.EDIT_INCOMES_CATEGORIES))
            buttonsHolder.getChildren().remove(editType);
        if (Authorization.cannot(AuthorizationKeys.DELETE_INCOMES_CATEGORIES))
    ...
    private void showTypeDialog(Type type) {
        try {
            ...
            stage.show();
            stage.setOnHidden(e -> typesTable.setItems(IncomeTypesRepository.getIncomeTypes()));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
