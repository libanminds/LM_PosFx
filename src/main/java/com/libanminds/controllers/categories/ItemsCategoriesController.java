...
import com.libanminds.controllers.dialogs.ItemCategoryDialogController;
...
public class ItemsCategoriesController implements Initializable {
    ...
    @FXML private TableView<ItemCategory> categoriesTable;
    ...
    private void handleAuthorization() {
        if (Authorization.cannot(AuthorizationKeys.ADD_ITEMS_CATEGORIES))
            buttonsHolder.getChildren().remove(addCategory);
        if (Authorization.cannot(AuthorizationKeys.EDIT_ITEMS_CATEGORIES))
            buttonsHolder.getChildren().remove(editCategory);
        if (Authorization.cannot(AuthorizationKeys.DELETE_ITEMS_CATEGORIES))
            buttonsHolder.getChildren().remove(deleteCategory);
    }

    private void initButtons() {
        addCategory.setOnMouseClicked((EventHandler<Event>) event -> showItemCategoryDialog(null));
        editCategory.setOnMouseClicked((EventHandler<Event>) event -> showItemCategoryDialog(selectedCategory));
    ...
    private void showItemCategoryDialog(ItemCategory category) {
        try {
            ...
            stage.show();
            stage.setOnHidden(e -> categoriesTable.setItems(ItemsCategoriesRepository.getItemsCategories()));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
